...
from r3lay.core.embeddings.mlx_text import DEFAULT_MODEL, MLXTextEmbeddingBackend
...
class TestVisionEmbeddingResult:
    ...
    def test_single_vector_result(self):
        ...
        result = VisionEmbeddingResult(
            vectors=vectors, dimension=768, embedding_type="single", num_vectors_per_image=1
        )
    ...
    def test_multi_vector_result(self):
        ...
        result = VisionEmbeddingResult(
            vectors=vectors, dimension=768, embedding_type="multi", num_vectors_per_image=256
        )
    ...
    def test_single_vector_invalid_3d(self):
        ...
        with pytest.raises(ValueError, match="Expected 2D array"):
            VisionEmbeddingResult(vectors=vectors, dimension=768, embedding_type="single")
    ...
    def test_single_vector_dimension_mismatch(self):
        ...
        with pytest.raises(ValueError, match="Dimension mismatch"):
            VisionEmbeddingResult(vectors=vectors, dimension=512, embedding_type="single")
    ...
    def test_multi_vector_dimension_mismatch(self):
        ...
        with pytest.raises(ValueError, match="Dimension mismatch"):
            VisionEmbeddingResult(
                vectors=vectors, dimension=1024, embedding_type="multi", num_vectors_per_image=256
            )
...
class TestVisionEmbeddingConfig:
    ...
    def test_custom_values(self):
        ...
        config = VisionEmbeddingConfig(max_image_size=1024, normalize=False, batch_size=8)
...
class TestMLXTextEmbeddingBackendLoad:
    ...
    async def test_load_failure_error_response(self):
        ...
        error_response = (
            json.dumps({"type": "loaded", "success": False, "error": "Model not found"}).encode()
            + b"\n"
        )
...
class TestMLXTextEmbeddingBackendEmbed:
    ...
    async def test_embed_texts_success(self):
        ...
        embed_response = (
            json.dumps(
                {"type": "embeddings", "vectors": encoded, "shape": [2, 384], "dtype": "float32"}
            ).encode()
            + b"\n"
        )
    ...
    async def test_embed_texts_error_response(self):
        ...
        # Error response
        error_response = json.dumps({"type": "error", "message": "Out of memory"}).encode() + b"\n"
...
class TestMLXTextEmbeddingBackendFullCycle:
    ...
    async def test_full_cycle(self):
        ...
        responses = [
            ...
            # Embed response
            json.dumps(
                {"type": "embeddings", "vectors": encoded, "shape": [3, 384], "dtype": "float32"}
            ).encode()
            + b"\n",
        ]
        response_idx = [0]
