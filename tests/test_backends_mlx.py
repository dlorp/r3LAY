...
from r3lay.core.backends.mlx import MLXBackend, MODEL_LOAD_TIMEOUT, SHUTDOWN_TIMEOUT
...
class TestMLXBackendInit:
    ...
    def test_is_loaded_true_when_process_running(self, backend: MLXBackend, mock_process: MagicMock):
        """Test is_loaded is True when process is running."""
        backend._process = mock_process
        mock_process.returncode = None
        assert backend.is_loaded is True

    def test_is_loaded_false_when_process_terminated(self, backend: MLXBackend, mock_process: MagicMock):
...
class TestMLXBackendLoad:
    ...
    @pytest.mark.asyncio
    async def test_load_process_dies_during_load(self, backend: MLXBackend, mock_process: MagicMock):
    ...
    @pytest.mark.asyncio
    async def test_load_vision_model_sends_flag(self, vision_backend: MLXBackend, mock_process: MagicMock):
...
class TestMLXBackendUnload:
    ...
    @pytest.mark.asyncio
    async def test_unload_force_terminate_on_timeout(self, backend: MLXBackend, mock_process: MagicMock):
    ...
    @pytest.mark.asyncio
    async def test_unload_kills_if_terminate_fails(self, backend: MLXBackend, mock_process: MagicMock):
...
class TestProcessManagement:
    ...
    @pytest.mark.asyncio
    async def test_terminate_process_kills_on_timeout(self, backend: MLXBackend, mock_process: MagicMock):
...
class TestWorkerCommunication:
    ...
    async def test_read_response_success(self, backend: MLXBackend, mock_process: MagicMock):
        ...
        mock_process.stdout.readline = AsyncMock(
            return_value=create_json_response(response_data)
        )
...
class TestGenerateStream:
    ...
    async def test_generate_stream_sends_correct_command(
        ...
        async for _ in backend.generate_stream(
            messages, max_tokens=256, temperature=0.5
        ):
            pass
    ...
    async def test_generate_stream_with_images(
        ...
        async for _ in vision_backend.generate_stream(
            messages, images=[image1, image2]
        ):
            pass
    ...
    async def test_generate_stream_error_from_worker(
        ...
        mock_process.stdout.readline = AsyncMock(
            return_value=create_json_response({
                "type": "error",
                "message": "Generation failed: context too long"
            })
        )
    ...
    @pytest.mark.asyncio
    async def test_generate_stream_process_dies(
        self, backend: MLXBackend, mock_process: MagicMock
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_stream_cancellation(
        self, backend: MLXBackend, mock_process: MagicMock
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_stream_empty_token(
        self, backend: MLXBackend, mock_process: MagicMock
    ):
